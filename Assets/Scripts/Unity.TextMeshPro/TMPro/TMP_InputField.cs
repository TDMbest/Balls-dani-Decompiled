...
namespace TMPro
{
	...
	public class TMP_InputField : Selectable, IUpdateSelectedHandler, IEventSystemHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler, ISubmitHandler, ICanvasElement, ILayoutElement, IScrollHandler
	{
		...
		private IEnumerator MouseDragOutsideRect(PointerEventData eventData)
		{
			...
			{
				Vector2 localPoint;
				RectTransformUtility.ScreenPointToLocalPointInRectangle(textViewport, eventData.position, eventData.pressEventCamera, out localPoint);
		...
		private void GenerateHightlight(VertexHelper vbo, Vector2 roundingOffset)
		{
			...
			AdjustRectTransformRelativeToViewport(startPosition, num2, true);
		...
		private void ActivateInputFieldInternal()
		{
			...
			if (TouchScreenKeyboard.isSupported && !shouldHideSoftKeyboard)
			{
				...
				{
					m_SoftKeyboard = ((inputType == InputType.Password) ? TouchScreenKeyboard.Open(m_Text, keyboardType, false, multiLine, true, false, "", characterLimit) : TouchScreenKeyboard.Open(m_Text, keyboardType, inputType == InputType.AutoCorrect, multiLine, false, false, "", characterLimit));
