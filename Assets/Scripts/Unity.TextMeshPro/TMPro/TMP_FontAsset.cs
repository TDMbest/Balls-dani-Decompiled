...
namespace TMPro
{
	...
	public class TMP_FontAsset : TMP_Asset
	{
		...
		public Texture2D[] atlasTextures
		{
			get
			{
				Texture2D[] atlasTexture2 = m_AtlasTextures;
		...
		public void ReadFontAssetDefinition()
		{
			...
			TMP_Character value;
			if (!m_CharacterLookupDictionary.ContainsKey(8209u) && m_CharacterLookupDictionary.TryGetValue(45u, out value))
			{
		...
		internal void SortCharacterTable()
		{
			...
			{
				m_CharacterTable = Enumerable.ToList(Enumerable.OrderBy(m_CharacterTable, (TMP_Character c) => c.unicode));
			}
		}
		...
		internal void SortGlyphTable()
		{
			...
			{
				m_GlyphTable = Enumerable.ToList(Enumerable.OrderBy(m_GlyphTable, (Glyph c) => c.index));
			}
		}
		...
		public bool HasCharacter(char character, bool searchFallbacks)
		{
			...
			TMP_Character character2;
			if (m_AtlasPopulationMode == AtlasPopulationMode.Dynamic && TryAddCharacterInternal(character, out character2))
			{
				return true;
			}
			if (searchFallbacks)
		...
		public bool TryAddCharacters(uint[] unicodes, out uint[] missingUnicodes)
		{
			...
			if (unicodes == null || unicodes.Length == 0 || m_AtlasPopulationMode == AtlasPopulationMode.Static)
			{
				...
				missingUnicodes = Enumerable.ToArray(unicodes);
				return false;
			}
			...
			for (int k = 0; k < m_CharactersToAdd.Count; k++)
			{
				...
				Glyph value;
				if (!m_GlyphLookupDictionary.TryGetValue(tMP_Character2.glyphIndex, out value))
				{
		...
		internal void UpdateAtlasTexture()
		{
			if (m_GlyphsToRender.Count != 0)
			{
				...
				int count = m_GlyphsToPack.Count;
				int num = 0;
			}
		}
		...
		public void ClearFontAssetData(bool setAtlasSizeToZero = false)
		{
			...
			if (m_AtlasTextures != null)
			{
				for (int i = 0; i < m_AtlasTextures.Length; i++)
				{
					...
					{
						texture2D.Resize(m_AtlasWidth, m_AtlasHeight, TextureFormat.Alpha8, false);
					}
		...
		private void UpgradeGlyphAdjustmentTableToFontFeatureTable()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				uint glyphIndex = 0u;
				TMP_Character value;
				if (m_CharacterLookupDictionary.TryGetValue(kerningPair.firstGlyph, out value))
				{
					glyphIndex = value.glyphIndex;
				}
				uint glyphIndex2 = 0u;
				TMP_Character value2;
				if (m_CharacterLookupDictionary.TryGetValue(kerningPair.secondGlyph, out value2))
				{
