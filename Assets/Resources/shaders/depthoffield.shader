	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1 = -_MainTex_TexelSize * float4(1.0, 0.0, 0.0, 1.0) + inp.texcoord.xyxy;
                tmp1.x = tex2D(_CoCTex, tmp1.xy);
                tmp0.z = tex2D(_CoCTex, tmp1.zw);
                tmp1.y = tmp1.x < tmp0.w;
                tmp2.z = tmp1.y ? tmp1.x : tmp0.w;
                ...
                tmp0.x = min(tmp1.x, tmp0.x);
                tmp0.x = tmp0.x - tmp0.w;
                o.sv_target = _TaaParams.zzzz * tmp0.xxxx + tmp0.wwww;
                return o;
			}
			ENDCG
		}
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.xyz = tex2D(_MainTex, tmp3.xy);
                tmp2.yzw = tex2D(_MainTex, tmp3.zw);
                tmp3.x = max(tmp1.z, tmp1.y);
                tmp3.x = max(tmp1.x, tmp3.x);
                tmp3.x = tmp3.x + 1.0;
                tmp4 = _MainTex_TexelSize * float4(-0.5, 0.5, 0.5, 0.5) + inp.texcoord1.xyxy;
                tmp3.y = tex2D(_CoCTex, tmp4.xy);
                tmp3.z = tex2D(_CoCTex, tmp4.zw);
                tmp3.z = tmp3.z * 2.0 + -1.0;
                tmp3.y = tmp3.y * 2.0 + -1.0;
                tmp3.x = abs(tmp3.y) / tmp3.x;
                tmp3.y = min(tmp3.z, tmp3.y);
                tmp0.xyz = tmp1.xyz * tmp3.xxx + tmp0.xyz;
                tmp0.w = tmp0.w + tmp3.x;
                tmp1.x = max(tmp2.w, tmp2.z);
                tmp1.x = max(tmp1.x, tmp2.y);
                tmp1.x = tmp1.x + 1.0;
                tmp1.x = abs(tmp3.z) / tmp1.x;
                tmp0.xyz = tmp2.yzw * tmp1.xxx + tmp0.xyz;
                tmp0.w = tmp0.w + tmp1.x;
                tmp0.w = max(tmp0.w, 0.00001);
                tmp0.xyz = tmp0.xyz / tmp0.www;
                tmp0.w = min(tmp1.w, tmp3.y);
                tmp1.x = max(tmp1.w, tmp3.y);
                tmp1.x = max(tmp1.x, tmp2.x);
                tmp0.w = min(tmp0.w, tmp2.x);
                tmp1.y = tmp1.x < -tmp0.w;
