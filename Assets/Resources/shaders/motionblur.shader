	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp3;
                float4 tmp4;
                tmp0 = tex2D(_History1LumaTex, inp.texcoord1.xy);
                tmp0.y = inp.texcoord1.x * _MainTex_TexelSize.z;
                tmp0.y = tmp0.y * 0.5;
                tmp0.y = floor(tmp0.y);
                tmp0.y = tmp0.y * 2.0 + 0.5;
                tmp1.z = tmp0.y * _MainTex_TexelSize.x + _MainTex_TexelSize.x;
                tmp1.x = tmp0.y * _MainTex_TexelSize.x;
                tmp1.yw = inp.texcoord1.yy;
                tmp2 = tex2D(_History1ChromaTex, tmp1.zw);
                tmp0.y = tmp2.x - 0.5;
                tmp2.xy = tmp0.yy * float2(1.402, 0.71414);
                tmp3 = tex2D(_History1ChromaTex, tmp1.xy);
                tmp0.y = tmp3.x - 0.5;
                tmp2.z = tmp0.y * -0.34414 + -tmp2.y;
                tmp2.w = tmp0.y * 1.772;
                tmp0.xyz = tmp0.xxx + tmp2.xzw;
                tmp2 = tex2D(_MainTex, inp.texcoord.xy);
                tmp0.xyz = tmp0.xyz * _History1Weight.xxx + tmp2.xyz;
                o.sv_target.w = tmp2.w;
                tmp2 = tex2D(_History2LumaTex, inp.texcoord1.xy);
                tmp3 = tex2D(_History2ChromaTex, tmp1.zw);
                tmp0.w = tmp3.x - 0.5;
                tmp3.xy = tmp0.ww * float2(1.402, 0.71414);
                tmp4 = tex2D(_History2ChromaTex, tmp1.xy);
                tmp0.w = tmp4.x - 0.5;
                tmp3.z = tmp0.w * -0.34414 + -tmp3.y;
                tmp3.w = tmp0.w * 1.772;
                tmp2.xyz = tmp2.xxx + tmp3.xzw;
                tmp0.xyz = tmp2.xyz * _History2Weight.xxx + tmp0.xyz;
                tmp2 = tex2D(_History3ChromaTex, tmp1.zw);
                tmp3 = tex2D(_History4ChromaTex, tmp1.zw);
                tmp0.w = tmp3.x - 0.5;
                tmp3.xy = tmp0.ww * float2(1.402, 0.71414);
                tmp0.w = tmp2.x - 0.5;
                tmp2.xy = tmp0.ww * float2(1.402, 0.71414);
                tmp4 = tex2D(_History3ChromaTex, tmp1.xy);
                tmp1 = tex2D(_History4ChromaTex, tmp1.xy);
                tmp0.w = tmp1.x - 0.5;
                tmp1.x = tmp4.x - 0.5;
                tmp2.z = tmp1.x * -0.34414 + -tmp2.y;
                tmp2.w = tmp1.x * 1.772;
                tmp1 = tex2D(_History3LumaTex, inp.texcoord1.xy);
                tmp1.xyz = tmp2.xzw + tmp1.xxx;
