	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                for (int i = tmp1.z; i < _SampleCount; i += 1) {
                    ...
                    tmp1.w = floor(tmp1.w);
                    tmp5.y = inp.texcoord.x * 0.0 + tmp1.w;
                    tmp2.x = tmp5.y * 78.233;
                    tmp2.x = sin(tmp2.x);
                    tmp2.x = tmp2.x * 43758.55;
                    tmp2.x = frac(tmp2.x);
                    tmp2.x = tmp0.w + tmp2.x;
                    tmp2.x = frac(tmp2.x);
                    tmp6.z = tmp2.x * 2.0 + -1.0;
                    tmp2.x = dot(tmp5.xy, float2(1.0, 78.233));
                    tmp2.x = sin(tmp2.x);
                    tmp2.x = tmp2.x * 43758.55;
                    tmp2.x = frac(tmp2.x);
                    tmp2.x = tmp0.w + tmp2.x;
                    tmp2.x = tmp2.x * 6.283185;
                    tmp2.x = sin(tmp2.x);
                    tmp7.x = cos(tmp2.x);
                    tmp3.w = -tmp6.z * tmp6.z + 1.0;
                    tmp3.w = sqrt(tmp3.w);
                    tmp7.y = tmp2.x;
                    tmp6.xy = tmp3.ww * tmp7.xy;
                    tmp1.w = tmp1.w + 1.0;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = 1.0 - tmp0.x;
                tmp0.x = log(tmp0.x);
                tmp0.x = tmp0.x * 0.4166667;
                tmp0.x = exp(tmp0.x);
                tmp0.x = tmp0.x * 1.055 + -0.055;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = tmp0.x * 1.055 + -0.055;
                tmp0.x = max(tmp0.x, 0.0);
                o.sv_target1.xyz = float3(1.0, 1.0, 1.0) - tmp0.xxx;
